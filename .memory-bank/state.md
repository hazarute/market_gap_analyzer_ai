...
- Ajanların kodlarındaki tüm varsayılan (default) prompt tanımları kaldırıldı. `OPPORTUNITY_MAP_PROMPT` ve `SYNTHESIS_PROMPT` değişkenleri `config.py` üzerinde `_get_required` ile zorunlu hale getirilerek testler ve `.env.example` bu doğrultuda güncellendi (23 Mayıs 2026).
